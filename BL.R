...
#Import metadata
metadatah = read.csv(file= "metadata_BL.csv", header=T, sep = ",")
head(metadatah)

# Reading counts data from featureCounts
counts <- read.csv("BL_star.csv", header=TRUE, sep = ",")
head(counts)
...
dds1 <- dds[ rowSums(counts(dds)) > 500, ]
nrow(dds1)
...
write.csv(as.data.frame(res_sig),"deseq2_BL.csv")
...
# Write final list to file
write.csv(as.data.frame(sig_pvalue), "edgeR_BL.csv")
...
#Write final list to file
write.csv(order_res,"limma_BL.csv")

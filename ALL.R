...
#Import metadata
metadatah = read.csv(file= "metadata_ALL.csv", header=T, sep = ",")
head(metadatah)

# Reading counts data from featureCounts
counts <- read.csv("ALL_star.csv", header=TRUE, sep = ",")
head(counts)
...
write.csv(as.data.frame(res_sig), "deseq2_ALL.csv")
...
#Write final list to file
write.csv(as.data.frame(sig_pvalue), "edgeR_ALL.csv")
...
#Write final list to file
write.csv(order_res,"limma_ALL.csv")

...
#Import metadata
metadatah = read.csv(file= "metadata_CLL.csv", header=T, sep = ",")
head(metadatah)

# Reading counts data from featureCounts
counts <- read.csv("CLL_star.csv", header=TRUE, sep = ",")
head(counts)
...
write.csv(as.data.frame(res_sig),"deseq2_CLL.csv")
...
#Write final list to file
write.csv(as.data.frame(sig_pvalue), "edgeR_CLL.csv")
...
#Write final list to file
write.csv(order_res,"limma_CLL.csv")
